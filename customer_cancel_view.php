...
<br>
  <center>
      <h3 class="text-center"><i class="fas fa-utensils"></i> <b>Order Cancelation</b></h3>
       <br><form method="post">
  ...
  $noti=mysqli_query($con,"SELECT * FROM notifications WHERE id='$cancel_ID' ");

  if(mysqli_num_rows($noti)>0)
  {
    while($notification=mysqli_fetch_array($noti))
    {
    ...
    ?>
               <tr>
		        			...
		        			<td><input type="submit" name="order_cancel" value="Cancel Order" class="btn btn-danger"></td>
		        		</tr>

                <?php } }
                    else
                    {
                        echo '<h4 class="text-center text-warning">NO Cancelation Orders Here...</h4>';
                    }
                    ?>
...
<!--End Dashboard Content-->
     <?php
      ...
      {
          $Update_cancel=mysqli_query($con,"UPDATE order_cancel SET status='1' WHERE Order_ID='$order_ID' AND Customer_ID='$Customer_ID' ");
          
          if($Update_cancel)
          {
              mysqli_query($con,"UPDATE notifications SET status='read' WHERE id='$cancel_ID' ");
              mysqli_query($con,"DELETE FROM delivery WHERE Order_ID='$order_ID' AND Customer_ID='$Customer_ID' ");
               echo '<script> alert("Food Successfully Cancelled..."); location.replace("Total_cancelled_Order.php"); </script>';
          }
          else
          {
              echo '<script> alert( "Food Not Cancelled..."); location.replace("customer_cancel_view.php"); </script>';
          }
      }
