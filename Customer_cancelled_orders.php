...
if(isset($_POST['order'])){
   ...
   {
     echo'<script> alert("product already added to cart"); location.replace("Customer_cancelled_orders.php");</script>';
   }
   
   else{
      $insert_product = mysqli_query($con, "INSERT INTO `cart`(ID,Name, Price,Qty,Image,Customer_ID) VALUES('','$product_name', '$product_price', '$product_quantity','$product_image','$customer_ID')");

      if($insert_product)
      {
         echo'<script> alert("product added to cart"); location.replace("cart.php");</script>';
      }
      else
      {
         echo'<script> alert("product not added to cart"); location.replace("cart.php");</script>';
      }
   }
}
          ...
          <li class="nav-item">
            ...
            <div class="collapse" id="form-elements">
              <ul class="nav flex-column sub-menu">
                ...
                <li class="nav-item"><a class="nav-link" href="Customer_cancelled_orders.php">Canceled Orders</a></li>
...
<li class="nav-item">
      ...
      <div class="main-panel">        
        <div class="content-wrapper">
            <h3 class="text-center"><b>Your Cancelled Orders</b></h3>
            <br>
              
                <?php 
            echo '<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">';
                            $tdy_order=mysqli_query($con,"SELECT * FROM order_cancel WHERE Customer_ID='$customer_ID' ORDER BY Date DESC");
            ...
            if(mysqli_num_rows($tdy_order)>0)
            {
                ...
                $P_date=$TODAY_ORDER['Date'];   
                $P_status=$TODAY_ORDER['status'];   

                 if($P_status==0)
                 {
                   $cancel_status='<p class="card-text text-center text-warning">Waiting For Cancel Process</p>';
                 }
                 else
                 {
                  $cancel_status="<p class='text-center text-warning'>Canceled</p>";
                 }
        ...
        <h4 class="cart-title text-center" style=" text-transform: capitalize;"><?php echo $P_NAME; ?> </h4>
       <img class="card-img-top" src="updated_photo/<?php echo $photo_fetch['image']; ?>" height="250">
                  <p class="card-text text-center"><?php echo $P_date; ?></p>
       ...
       <!-- content-wrapper ends -->
        ...
        <footer class="footer">
          ...
          <div class="d-sm-flex justify-content-center justify-content-sm-between">
            <span class="text-muted text-center text-sm-left d-block d-sm-inline-block">Developed by <a href="developer.php" target="_blank">VTA Students Vanthaaramulai</a></span> 
          </div>
        </footer> 
        <!-- partial -->
      </div>
