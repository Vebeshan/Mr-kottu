...
if(isset($_POST['add_to_cart'])){
   ...
   if(mysqli_num_rows($select_cart) > 0)
   {
     $error_msg[]= 'This Food Already Added To Your Cart !!!';
      ...
      $insert_product = mysqli_query($con, "INSERT INTO `cart`(ID,Name, Price, Qty,Image,Customer_ID) VALUES('','$product_name', '$product_price', '$product_quantity','$product_image','$user_id')");
    ...
    <section id="menu" class="menu">
      <div class="container-fluid">
        ...
        <div class="section-title">
            <h2><i class="fas fa-search"></i> Search <span> Results...</span></h2>
        </div>
       ...
       if(isset($_POST['search']))
          {
              ...
              $search=mysqli_query($con,"SELECT * FROM products WHERE Product_Name  LIKE '%$mysearch%' OR Type LIKE '%$mysearch%'");
              
              if(mysqli_num_rows($search)>0)
              {
                  while($fetch_prodcut=mysqli_fetch_assoc($search))
         {
          ...
          if($fetch_prodcut['status']==1)
          {
            ...
            $button='<button type="submit" class="btn" name="add_to_cart"><i class="fas fa-shopping-cart"></i> add to cart</button>';
          }
          else
          {
            ...
            $button='<button type="submit" class="btn btn-danger" name="add_to_cart" disabled><i class="fas fa-shopping-cart"></i> add to cart</button>';
          }
   ?>
   
         <div class="box">
             <h2><span><?php echo $status; ?></span></h2>
              
            <img src="updated_photo/<?php echo $fetch_prodcut['image']; ?>" class="image" data-cat="<?php echo $name; ?>" data-search="<?php echo $name; ?>" alt="Food">
      ...
      <?php
         ...
         {
            echo '<h1 class="empty text-center text-warning"><i class="fas fa-search"></i> Search Results Not Found !</h1>';
         };
...
<center><a href="foodmenu.php" style="padding:10px 15px; background-color: #f39c12; color:#fff; border-radious:25%;">Back To Full Menu</a></center>
