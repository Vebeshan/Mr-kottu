...
<?php
   ...
   if($delete_query){
      echo "<script> alert('Product has been deleted'); location.replace('product.php'); </script>";
   }
     else
   {
      echo "<script> alert('Product could not be deleted'); location.replace('product.php'); </script>";
   }
}      
          ?>
  ...
  <br>
   <div class="container align-items-center justify-content-center shadow-sm">
      <div class="row">
         ...
         <div class="card">
            <div class="card-body">
              <div class="card-title text-center"><h3><i class="fas fa-utensils"></i> Add Product</h3></div>
               <div class="table-responsive">
               <table class="table">
            <form method="post" enctype="multipart/form-data">
               <table width="100%" align="center" cellpadding="5" cellspacing="5" class="table-Striped">
                  <tr>
                     <th>
                        Product Name
                         <input type="text" name="xpname" placeholder="Enter Product Name " class="form-control" required="required">
                     </th>
                 </tr>
                 ...
                 <tr>
                  <th>
                      Type
                      <select name="menu_type" class="form-control input-shadow" required>
                         <option value=" ">Please Select...</option>
                         <option value="Kottu">Kottu</option>
                         <option value="Cheese Kottu">Cheese Kottu</option>
                         <option value="Barotta">barota</option>
              ...
              <tr>
               <th>
                ...
                minlength="3" name="xprice" placeholder="Enter Price " class="form-control" required="required">
               </th>
           </tr>
           ...
           <tr>
               <th>
                  Product Image
                   <input type="file" name="image" placeholder="Select An Image " class="form-control" required="required" accept="image/jpg, image/png, image/jpeg">
               </th>
           </tr>
   ...
   <?php
          ...
          {
            $pname=$_POST['xpname'];
            $ptype=$_POST['menu_type'];
            $price=$_POST['xprice'];


            $pimage=$_FILES['image'] ['name'];
            $pimage_tmp_name = $_FILES['image']['tmp_name'];
            $pimage_folder = 'updated_photo/'.$pimage;

            $select="SELECT * FROM products WHERE Product_Name='$pname' ";
            $execute=mysqli_query($con,$select);

            if(mysqli_num_rows($execute) >0)
            {
              echo "<script> alert ('This Product Already Added So, You Cannot Add This Product Again, Want To Make Changes From This Product Click Update');</script>";
            }
            else
            {
              $sql="insert into products(Product_Name,Type,Price,image,DATE,status) values('$pname','$ptype',' $price','$pimage',NOW(),'1')";
             ...
             {
              echo "<script> alert ('This Product Already Added So You Cannot Add Again, Want To Make Changes From This Product Click Update');</script>";
