...
if(isset($_GET['did']))
{
	...
	$sql="delete from faq where ID=$did ";

	$res=mysqli_query($con,$sql);
	if($res)
	{
		echo "<script> alert('FAQ Deleted'); location.replace('home_page_setting.php'); </script>";
	}
	else
	{
...
if(isset($_POST['update']))
{
  ...
  $uimage_tmp_name = $_FILES['image']['tmp_name'];
  $uimage_folder = 'updated_photo/'.$uimage;
    
    $update=mysqli_query($con,"UPDATE home_page SET About='$about',About_banner='$uimage',Term='$term',Policy='$policy' ");
    
    if($update)
    {
        echo "<script> alert('Settings Updated'); location.replace('home_page_setting.php'); </script>";
    }
  ...
  <div class="content-wrapper">
    <div class="container-fluid">
        <h3 class="text-center">Home Page Settings</h3>
                ...
                <a href="New_FAQ.php" class="btn btn-primary">Add FAQ</a>
        <br><br>
        <table class="table table-responsive table-bordered" width="100%" align="center">
            <tr>
                <thead>
                    <th>No</th>
                     <th>Question</th>
    ...
    $faq=mysqli_query($con,"SELECT * FROM faq");
    if(mysqli_num_rows($faq)>0)
    {
        $x=1;
        while($faq_fetch=mysqli_fetch_array($faq))
        {
            $id=$faq_fetch['ID'];
            ?>
            ...
            <tbody>
                <tr>
                    <td><?php echo $x++; ?></td>
                     <td><?php echo $faq_fetch['Question']; ?></td>
                     <td><?php echo $faq_fetch['Answer']; ?></td>
                    <td cols-2>
                    <a <?php echo 'href="Update_FAQ.php?uid='.$id.' "'; ?> class="btn btn-md btn-success" ><i class="fas fa-edit"></i>Update</a>
  <a <?php echo 'href="home_page_setting.php?did='.$id.' "'; ?> class="btn btn-md btn-danger" onclick="return confirm('Are Your Sure You Want To Delete This FAQ?');"><i class="fas fa-trash"></i>Delete</a> 
        ...
        $home=mysqli_query($con,"SELECT * FROM home_page");
        $home_fetch=mysqli_fetch_array($home);
        ...
        <form method="post" enctype="multipart/form-data">
            <br><br>
            <label>Mr Kottu Policy</label>
            <textarea name="policy" class="form-control" required rows="6" placeholder="Enter Your Policy... "><?php echo $home_fetch['Policy']; ?> </textarea>
            <br>
            <label>Mr Kottu TERMS & CONDITIONS</label>
            <textarea name="term" class="form-control" required rows="6" placeholder="Enter Your TERMS & CONDITIONS... "><?php echo $home_fetch['Term']; ?></textarea>
            <br>
            <label>About</label>
            <textarea name="about" class="form-control" required rows="6" placeholder="Enter Your TERMS & CONDITIONS... "><?php echo $home_fetch['About']; ?></textarea>
            <br>
            <label>Banner</label>
            <input type="file" name="image" accept="image/jpeg" class="form-control" value="<?php echo $home_fetch['About_banner']; ?>" required>
            <br>
            <center><button type="submit" name="update" class="btn btn-primary"><i class="fas fa-edit"></i> Update </button>
