...
mysqli_query($con,"UPDATE notifications SET status='read' WHERE type='ORDER' ");
...
<br>
  <center>
       <h3 class="text-center"><i class="fas fa-bell"></i><b> Notification History</b></h3>
       <br>
  ...
  $noti=mysqli_query($con,"SELECT * FROM `notifications` ORDER BY date desc ");
  ...
  if(mysqli_num_rows($noti)>0)
  {
    while($notification=mysqli_fetch_array($noti))
    {
        ...
        {
            $notidetail="<button class='btn btn-warning'>Order</button>";
        }
        else if($notification['type']=="Delivered")
        {
            $notidetail="<button class='btn btn-success'>Delivered</button>";
        }
        else
        {
                if($notification['type']=="Cancel Order")
            {
                $notidetail="<button class='btn btn-danger'>canceled</button>";
            }
        }
        
        if($notification['status']=="read")
        {
            $read="<button class='btn btn-success'>Read</button>";
        }
        else
        {
            $read="<button class='btn btn-danger'>Unread</button>";
        }
