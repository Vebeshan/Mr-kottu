	...
	foreach($_FILES['images']['name'] as $key=>$val){
		...
		if(in_array($fileType, $allowTypes)){	
			...
			if(move_uploaded_file($_FILES['images']['tmp_name'][$key],$targetFilePath)){
				...
				$insert =mysqli_query($con,"INSERT into gallery (Title,Name,Date) VALUES ( '$food','".$targetFilePath."', NOW())");
...
?>
       ...
       <div class="container">
		<h3 class="text-center"> <i class="fas fa-images"></i> Gallery Upload </h3>
				...
				<form action="" method="post" enctype="multipart/form-data">
                 ...
                 <select name="food" class="form-control">
                        ...
                        <option value="Cheese_Kottu" >Cheese Kottu</option>
                 ...
                 <input type="file" name="images[]" class="form-control" multiple required>
                         ...
                         <button type="submit" name="submit" class="btn btn-md btn-info">
                             <i class="fas fa-file-upload"></i> upload</button><br>
				</form>
			</div>

	<!-- gallery view of uploaded images --> 
	<div class="container shadow-sm">
				<h4 class="text-center">Uploaded Images</h4><br>
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3 text-center">
				<?php 
					$query = mysqli_query($con,"SELECT * FROM gallery ORDER BY Date DESC");
					if($query->num_rows > 0){
        ...
        <div class="col-md-3">
            <div class="card">
                <div class="card-body">
                <img class="card-img" src="<?php echo $imageURL; ?>" width="200" height="200" alt="" />
                <a <?php echo 'href="Galary.php?did='.$imageid.' "'; ?> class="btn btn-md btn-danger" onclick="return confirm('are your sure you want to delete this?');"><i class="fas fa-trash"></i> Delete</a>  
            </div></div></div>
							
						<?php }
					}else{ ?>
						<p class="text-center">No image(s) found...</p>
				<?php } ?>
            </div>
			</div>
	</div>
...
if(isset($_GET['did']))
{
	...
	if($res)
	{
       echo " <script> location.replace('Galary.php'); </script>";
	}
	else
	{
