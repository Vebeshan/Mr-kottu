...
if(isset($_GET['did'])){
   $delete_id = $_GET['did'];
   $delete_query = mysqli_query($con, "DELETE FROM `message` WHERE ID = $delete_id ") or die('query failed');
   if($delete_query){
      echo "<script> alert('Message Deleted Successfully...'); location.replace('Messages.php'); </script>";
   }else{
       echo "<script> alert('Message Not Deleted...'); location.replace('Messages.php');</script>";
   };
};
...
<div class="clearfix"></div>
  ...
  <center>
      ...
      <div class="col-lg-12">
             <div class="card">
             <div class="card-body">
                  <h2 class="card-title" style="font-size:26px;"><B><i class="fas fa-envelope"></i> Messages</B></h2>
       <br>
...
if (mysqli_num_rows($res) > 0) {
    ...
    $mail = $row['Email'];
             ...
             if($row['status']=='0')
             {
                 $msg_status='<small class="badge float-right badge-light text-success text-center">New</small>';
             }
             else
             {
                 $msg_status='<small class="badge float-right badge-light text-warning text-center">Old</small>';
             }
    ?>
               ...
               <tr>
		        			...
		        			<td><?= $row['message']; ?></td>
		        			<td>
<a <?php echo 'href="reply.php?uid=' . $id . ' "'; ?> class="btn btn-md btn-success"><i class="fas fa-reply"></i> Replay</a>
                                
 <a <?php echo 'href="Messages.php?did=' . $id . ' "'; ?> class="btn btn-md btn-danger" onclick="return confirm('Are You Want To Delete This Message ?');"><i class="fas fa-trash"></i> Delete</a>
...
{
  echo '<h4 class="text-center"> No Messages Available...</h4>';
}
